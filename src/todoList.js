...
export class Todo extends Component {
  ...
  addNewTodo(event) {
    ...
    let newTodo = {name : event.target['newTask'].value , done:false}
  ...
  render() {
    ...
    let todoItems = this.state.todoList.map(function(item,index) {
          return (
            <TodoItems name={item.name} done={item.done} onChange={() => self.changeTaskStatus(index)} deleteTask={() => self.deleteTask(index)} />
          );
        });
  return (
    <div>
